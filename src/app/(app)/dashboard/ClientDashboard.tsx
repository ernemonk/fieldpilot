...
const myJobs: { title: string; status: JobStatus; lastUpdate: string; progress: number }[] = [
  { title: 'Electrical Panel Upgrade', status: 'in_progress', lastUpdate: 'Updated 2 hours ago', progress: 65 },
  { title: 'Generator Maintenance', status: 'completed', lastUpdate: 'Completed Feb 10', progress: 100 },
  { title: 'Lighting Install – Phase 2', status: 'scheduled', lastUpdate: 'Starts Feb 20', progress: 0 },
];

const myProposals: { title: string; amount: string; status: ProposalStatus; date: string }[] = [
  { title: 'Office Rewiring Project', amount: '$12,500', status: 'sent', date: 'Feb 13, 2026' },
  { title: 'Emergency Lighting', amount: '$4,200', status: 'approved', date: 'Feb 8, 2026' },
  { title: 'Solar Panel Install', amount: '$28,000', status: 'rejected', date: 'Jan 30, 2026' },
];
  ...
  return (
    <div>
      ...
      <div className="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard
          label="Active Projects"
          value={myJobs.filter((j) => j.status === 'in_progress' || j.status === 'scheduled').length}
          icon={<Briefcase className="h-5 w-5" />}
        />
        <StatCard
          label="Proposals"
      ...
      <div className="mb-8 space-y-4">
        {myJobs.map((job, i) => (
          <Link key={i} href="/jobs" className="block">
            <div className="rounded-xl border border-[#E5E7EB] bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-base font-semibold text-[#111827]">{job.title}</p>
                  <p className="mt-1 text-xs text-[#6B7280]">{job.lastUpdate}</p>
                </div>
                <StatusBadge status={job.status} />
              </div>
              {/* Progress bar */}
              <div className="mt-4">
                <div className="mb-1 flex items-center justify-between">
                  ...
                  <span className="text-xs font-medium text-[#111827]">{job.progress}%</span>
                </div>
      ...
      <Card>
        ...
        <div className="space-y-3">
          {myProposals.map((p, i) => (
            <Link key={i} href="/proposals" className="block">
              <div className="flex items-center justify-between rounded-lg border border-[#E5E7EB] p-4 transition-colors hover:bg-gray-50">
                <div>
                ...
                <div className="flex items-center gap-3">
                  <span className="text-sm font-semibold text-[#111827]">{p.amount}</span>
                  <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${proposalStatusColor[p.status]}`}>
                    {proposalStatusLabel[p.status]}
                  </span>
                </div>
